...
for fi=1:length(frex)
    ...
    for ti=1:size(tf_all2all,4)
        ...
        temp = temp>thresh;         % threshold and binarize
...
connmat = squeeze(tf_all2all(:,:,dsearchn(frex',freq2use),dsearchn(times2save',time2use)));
thresh  = median(tempsynch) + std(tempsynch);
connmat = connmat + triu(connmat)';  % make symmetric matrix
connmat = connmat>thresh;            % threshold and binarize
...
for threshi=1:length(thresholds)
    ...
    connmat = squeeze(tf_all2all(:,:,dsearchn(frex',freq2use),dsearchn(times2save',time2use)));
    thresh  = median(tempsynch) + std(tempsynch)*thresholds(threshi);
    connmat = connmat + triu(connmat)';  % make symmetric matrix
    connmat = connmat>thresh;            % threshold and binarize
