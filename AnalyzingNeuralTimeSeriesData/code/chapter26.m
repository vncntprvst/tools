...
ispc    = bsxfun(@minus,ispc,mean(ispc(:,baselineidx(1):baselineidx(2)),2)); % not plotted in the book, but you can plot it for comparison with PLI
ispc_t  = bsxfun(@minus,ispc_t,mean(ispc_t(:,baselineidx(1):baselineidx(2)),2));
pli     = bsxfun(@minus,pli,mean(pli(:,baselineidx(1):baselineidx(2)),2));
dwpli   = bsxfun(@minus,dwpli,mean(dwpli(:,baselineidx(1):baselineidx(2)),2));
